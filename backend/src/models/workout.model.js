...
export const getWorkoutByIdAndUserId = async (workoutId, userId) => {
  const res = await query(
    `SELECT id, user_id, type, duration_minutes, calories_burned, notes, workout_date, created_at, updated_at
...
export const updateWorkout = async (workoutId, userId, { type, duration_minutes, calories_burned, notes, workout_date }) => {
  const res = await query(
    `UPDATE workouts
     SET type = $1, duration_minutes = $2, calories_burned = $3, notes = $4, workout_date = $5
     WHERE id = $6 AND user_id = $7 RETURNING *`,
    [type, duration_minutes, calories_burned, notes, workout_date, workoutId, userId]
  );
  return res.rows[0];
};
...
export const deleteAllWorkoutsByUserId = async (userId) => {
